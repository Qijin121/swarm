...
def convert_bbox_to_z(bbox):
    ...
    Takes a bounding box in the form [x1,y1,x2,y2,depth] and returns z in the form
      [x,y,s,r,d] where x,y is the centre of the box, s is the scale/area, r is
      the aspect ratio, and d is depth
    """
    w = bbox[2] - bbox[0]
    h = bbox[3] - bbox[1]
    x = bbox[0] + w/2.
    ...
    r = w / float(h+1e-6)
    d = bbox[4]  # depth information
    return np.array([x, y, s, r, d]).reshape((5, 1))


def convert_x_to_bbox(x, score=None):
    """
    Takes a bounding box in the centre form [x,y,s,r,d] and returns it in the form
      [x1,y1,x2,y2,d] where x1,y1 is the top left, x2,y2 is the bottom right, and d is depth
    """
    w = np.sqrt(x[2] * x[3])
    h = x[2] / w
    if(score == None):
...
def speed_direction(bbox1, bbox2):
    cx1, cy1, d1 = (bbox1[0]+bbox1[2])/2.0, (bbox1[1]+bbox1[3])/2.0, bbox1[4]
    cx2, cy2, d2 = (bbox2[0]+bbox2[2])/2.0, (bbox2[1]+bbox2[3])/2.0, bbox2[4]
    speed = np.array([cy2-cy1, cx2-cx1, d2-d1])
    norm = np.sqrt((cy2-cy1)**2 + (cx2-cx1)**2 + (d2-d1)**2) + 1e-6
    return speed / norm
...
class KalmanBoxTracker(object):
    ...
    def __init__(self, bbox, delta_t=3, orig=False):
        """
        Initialises a tracker using initial bounding box with depth information.
        """
        # define constant velocity model
        if not orig:
            from kalmanfilter import KalmanFilterNew as KalmanFilter
            self.kf = KalmanFilter(dim_x=9, dim_z=5)  # Added depth dimension
        else:
        ...
        self.kf.F = np.array([
            ...
            [0,0,1,0,0,0,0,1,0],  # s
            [0,0,0,1,0,0,0,0,0],  # r
            [0,0,0,0,1,0,0,0,1],  # d (depth)
            [0,0,0,0,0,1,0,0,0],  # dx
            [0,0,0,0,0,0,1,0,0],  # dy
            [0,0,0,0,0,0,0,1,0],  # ds
            [0,0,0,0,0,0,0,0,1]   # dd (depth velocity)
        ])
        ...
        self.hit_streak = 0
        self.age = 0
        self.last_observation = np.array([-1, -1, -1, -1, -1, -1])  # Added -1 for depth
...
class OCSort(object):
    def __init__(self, det_thresh, max_age=30, min_hits=1, 
    ...
    def update(self, output_results, img_info, img_size):
        """
        Params:
          output_results - a numpy array of detections in the format [[x1,y1,x2,y2,depth,score],...]
        """
        if output_results is None:
            return np.empty((0, 6))  # Changed to 6 to include depth

        self.frame_count += 1
        
        # post_process detections
        if output_results.shape[1] == 6:  # Changed to 6 to include depth
            scores = output_results[:, 5]
            bboxes = output_results[:, :5]  # Include depth information
        else:
            output_results = output_results.cpu().numpy()
            scores = output_results[:, 5] * output_results[:, 6]  # Adjusted index
            bboxes = output_results[:, :5]  # Include depth information

        img_h, img_w = img_info[0], img_info[1]
        scale = min(img_size[0] / float(img_h), img_size[1] / float(img_w))
        bboxes[:, :4] /= scale  # Only scale x,y coordinates, not depth
        ...
        for trk in self.trackers:
            velocity = trk.velocity if trk.velocity is not None else np.array((0, 0,0))
            print(velocity)


        velocities = np.array(
            [trk.velocity if trk.velocity is not None else np.array((0, 0,0)) for trk in self.trackers])
    ...
    def update_depth(self, dets, cates, scores):
            self.frame_count += 1
