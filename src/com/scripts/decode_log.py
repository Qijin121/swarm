...
class DecoderNode:
    ...
    def init_data_logging(self):
        """初始化数据记录"""
        # 创建数据存储目录
        self.log_dir = os.path.join(os.path.expanduser('~'), 'decoder_logs')
        os.makedirs(self.log_dir, exist_ok=True)
        
        # 创建日志文件名（使用时间戳）
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        self.command_log = os.path.join(self.log_dir, f'command_log_{timestamp}.csv')
        
        # 初始化CSV文件
        with open(self.command_log, 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(['timestamp', 'tracking_id', 'type', 'data', 'x_pos', 'y_pos'])
        ...
        while not rospy.is_shutdown():
            try:
                ...
                if len(buffer) >= buffer_size or (buffer and current_time - last_write_time >= write_interval):
                    with open(self.command_log, 'a', newline='') as f:
                        writer = csv.writer(f)
    ...
    def log_command(self, tracking_id, command, x_pos, y_pos):
        """记录发布的命令（非阻塞方式）"""
        timestamp = rospy.Time.now().to_sec()
        try:
            self.log_queue.put_nowait([timestamp, tracking_id, 'command', command, x_pos, y_pos])
        except queue.Full:
            rospy.logwarn("Log queue is full, dropping log entry")

    def log_signal(self, tracking_id, signal_type, signal_data, x_pos, y_pos):
        """记录信号序列（非阻塞方式）"""
        timestamp = rospy.Time.now().to_sec()
        try:
            self.log_queue.put_nowait([timestamp, tracking_id, signal_type, signal_data, x_pos, y_pos])
        except queue.Full:
            rospy.logwarn("Log queue is full, dropping log entry")
    ...
    def decode_and_publish(self, tracking_id, signal, last_x, last_y):
        ...
        """
        # 记录原始信号序列
        self.log_signal(tracking_id, 'raw_signal', str(signal), last_x, last_y)
            ...
            if len(decoded_messages) == 3:
                ...
                if len(set(decoded_messages)) == 3:
                    ...
                    with self.lock:
                        ...
                        # 记录发布的命令
                        self.log_command(tracking_id, most_common_value, last_x, last_y)
    ...
    def decode_signal(self, signal):
        ...
        # 记录处理后的信号序列
        self.log_signal(signal[0], 'processed_signal', str(processed_signal), signal[1], signal[2])
